...
void GpuMiner::setChallengeNumber(std::string const& challengeNumber)
{
  set(&Solver::setChallenge, challengeNumber);
}

void GpuMiner::setDifficultyTarget(std::string const& difficultyTarget)
{
  set(&Solver::setTarget, difficultyTarget);
}

void GpuMiner::setMinerAddress(std::string const& minerAddress)
{
...
void GpuMiner::thr_func(Solver& solver)
{
  std::random_device r;
...
void GpuMiner::solutionFound(Solver::bytes_t const& solution)
{
  {
...
void GpuMiner::set(void (Solver::*fn)(std::string const&), std::string const& p)
{
  for (auto&& i : m_solvers)
    (i.*fn)(p);
}

std::string GpuMiner::solution() const
{
