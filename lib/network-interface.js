  ...
  async sendMiningSolutions()
    {
      ...
      if(this.busySendingSolution == false)
      {
        if(queueMiningSolution.length > 0)
        {
          this.busySendingSolution = true;
          var nextSolution = queueMiningSolution.pop();

            console.log('popping mining solution off stack ')
          var response = await submitMiningSolution(nextSolution.addressFrom,
            nextSolution.solution_number, nextSolution.challenge_digest);

          console.log(response)
          this.busySendingSolution = false;
        }
      }
