...
module.exports =  {
  ...
  async requirePassword(require)
  {
    ...
    async init(web3,miningLogger)
    {
     ...
     if( requirePassword && userPassword == null ){
      ...
      if(!this.vaultDataExists(localStorage) )
      {
        ...
        localStorage.setItem('vaultdata', JSON.stringify({data:  this.getInitialVaultDataJson()  } ) )
   ...
   async handleAccountCommand(subsystem_command,subsystem_option)
    ...
    {
      //vaultData
      var vaultData = JSON.parse(localStorage.getItem('vaultdata') );

      var encryptedVaultData = vaultData.data;

      var vaultVersion = vaultData.version;

      if(vaultVersion == null){vaultVersion = 1;}
      ...
      if(vaultVersion == 1)
      {
          ...
          vaultData = JSON.parse(cryptr.decrypt( encryptedVaultData  ));
      ...
      {
         vaultData = JSON.parse(  encryptedVaultData   );

         if(requirePassword){ //we only do this in certain modes
           vaultData.privateKey = cryptr.decrypt( vaultData.cryptedPrivateKey )
         }
    ...
    saveVaultData( vData )
    {
      ...
      for(var i in vData.account_list)
      {
        ...
        {
          if( !this.hasValidPassword() ) //require password must be true !
          {
            console.error('Error when saving the VaultData! It could not be saved without a password. ')
            return
          }

          console.log("encrypting private key for ", vData.account_list[i])
        ...
        console.log( 'vaultData',vData  )
      console.log( JSON.stringify(vData) )

      localStorage.setItem('vaultdata', JSON.stringify({data: vData, version: VAULT_DATA_VERSION   } ) )
