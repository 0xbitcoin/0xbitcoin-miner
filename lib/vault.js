    ...
    getTokenContractAddress()
    {
      ...
      if( typeof vaultData.selected_contract_address != undefined )
      {
        ...
        if( web3Utils.isAddress(vaultData.selected_contract_address) )
        {
          ...
          console.log('Your selected contract address is invalid.  Please select a valid PoW token contract to mine into with "npm run contract select 0x####"')
          process.exit();
        }

      }else{
        console.log('Please select a PoW token contract to mine into with "npm run contract select 0x####"')
        process.exit();
      }
    },
